...
import { readLocalFile, readLocalFileCsv } from "../../utils/file";
...
const ImportForm = ({ models }) => {
  ...
  const upload = () => {
    ...
    switch (filetype) {
      // switch, because more can come ...
      case "text/csv":
        setLoading(true);
        // const res = await readLocalFileCsv(sourceFile)
        // console.log('res', res)

        readLocalFileCsv(sourceFile)
          .then((result) => {
  ...
  return (
    <div>
      <FieldRow>
        <label htmlFor="source">Content Source File (json or csv)</label>
        <FileField>
          <input
            id="source"
            name="source"
